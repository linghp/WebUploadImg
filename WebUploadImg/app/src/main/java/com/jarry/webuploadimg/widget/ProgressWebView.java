...
public class ProgressWebView extends WebView {
  ...
  @SuppressWarnings("deprecation") private void init(final Context context) {
        ...
        if (which == 0) {
          if (ContextCompat.checkSelfPermission(context, Manifest.permission.CAMERA)
            ...
            ActivityCompat.requestPermissions((Activity) context,
                new String[] { Manifest.permission.CAMERA }, TYPE_REQUEST_PERMISSION);
          } else {
            toCamera();
          }
        } else {
  ...
  private static Uri getOutputMediaFileUri() {
    return Uri.fromFile(
        FileManager.getImgFile(MyApplication.getInstance().getApplicationContext()));
  }
